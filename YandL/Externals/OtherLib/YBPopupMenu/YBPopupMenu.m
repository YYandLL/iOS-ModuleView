...
@property (nonatomic, assign) CGFloat ybX;
@property (nonatomic, assign) CGFloat ybY;
                      ...
                      delegate:(id<YBPopupMenuDelegate>)delegate
{
    if (self = [super init]) {
        ...
        self.txby_width = itemWidth;
        self.txby_height = (titles.count > 5 ? 5 * kButtonHeight : titles.count * kButtonHeight) + 2 * kArrowHeight;
        
        kArrowPosition = 0.5 * self.txby_width - 0.5 * kArrowWidth;
        ...
        _contentView.txby_height -= 2 * kArrowHeight;
        _contentView.txby_centerY = _mainView.txby_centerY;
...
- (void)showRelyOnView:(UIView *)view
{
    ...
    if (self.ybY < _anchorPoint.y) {
        self.ybY -= absoluteRect.size.height;
    }
    [self show];
}
...
- (void)setOffset:(CGFloat)offset
{
    ...
    self.ybY += self.ybY >= _anchorPoint.y ? offset : -offset;
}
...
- (void)determineAnchorPoint
{
    ...
    if (CGRectGetMaxY(self.frame) > kScreenHeight) {
        aPoint = CGPointMake(fabs(kArrowPosition) / self.txby_width, 1);
    }else {
        aPoint = CGPointMake(fabs(kArrowPosition) / self.txby_width, 0);
    }
...
- (CAShapeLayer *)getMaskLayerWithPoint:(CGPoint)point
{
    ...
    if (CGRectGetMaxY(self.frame) > kScreenHeight) {
        ...
        kArrowPosition = self.txby_width - kArrowPosition - kArrowWidth;
        layer = [self drawMaskLayer];
        layer.affineTransform = CGAffineTransformMakeRotation(M_PI);
        self.ybY = _anchorPoint.y - self.txby_height;
    }
    self.ybY += self.ybY >= _anchorPoint.y ? _offset : -_offset;
    return layer;
}
...
- (void)setArrowPointingWhere: (CGPoint)anchorPoint
{
    ...
    self.ybX = anchorPoint.x - kArrowPosition - 0.5*kArrowWidth;
    self.ybY = anchorPoint.y;
    
    CGFloat maxX = CGRectGetMaxX(self.frame);
    CGFloat minX = CGRectGetMinX(self.frame);
    
    if (maxX > kScreenWidth - 10) {
        self.ybX = kScreenWidth - 10 - self.txby_width;
    }else if (minX < 10) {
        self.ybX = 10;
    }
    ...
    if ((anchorPoint.x <= maxX - kCornerRadius) && (anchorPoint.x >= minX + kCornerRadius)) {
        ...
        kArrowPosition = self.txby_width - kCornerRadius - kArrowWidth;
    }
}
...
- (CAShapeLayer *)drawMaskLayer
{
    ...
    CGPoint topRightArcCenter = CGPointMake(self.txby_width-kCornerRadius, kArrowHeight+kCornerRadius);
    CGPoint topLeftArcCenter = CGPointMake(kCornerRadius, kArrowHeight+kCornerRadius);
    CGPoint bottomRightArcCenter = CGPointMake(self.txby_width-kCornerRadius, self.txby_height - kArrowHeight - kCornerRadius);
    CGPoint bottomLeftArcCenter = CGPointMake(kCornerRadius, self.txby_height - kArrowHeight - kCornerRadius);
    ...
    [path addLineToPoint: CGPointMake(self.txby_width-kCornerRadius, self.txby_height - kArrowHeight)];
    [path addArcWithCenter: bottomRightArcCenter radius: kCornerRadius startAngle: -M_PI-M_PI_2 endAngle: -M_PI*2 clockwise: NO];
    [path addLineToPoint: CGPointMake(self.txby_width, kArrowHeight+kCornerRadius)];
